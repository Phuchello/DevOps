...
From the repository root, a clean machine needs the package dependencies and a
Playwright Chromium browser. `npm ci` does not provision the browser binary:

```bash
cd fieldbook/vol1-linux
npm ci
npx playwright install chromium
npm run check:frozen-sources
npm run build:chapter02
npm run check:reader-quality
```
...
The Chapter 02 builder supports two explicit modes. The default `--mode=standalone` includes one chapter cover and one chapter TOC for a standalone proof artifact. A future volume builder may use `--mode=volume`, which suppresses both so the volume owns exactly one cover and one volume TOC. The volume cover must retain `BIÊN SOẠN: VÕ TRỌNG PHÚC`.
